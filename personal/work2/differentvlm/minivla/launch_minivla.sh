#!/bin/bash
# Launch MiniVLA experiment pipeline
# Usage: bash launch_minivla.sh --selection-mode <mode> --num-episodes <num> --dataset-name <name> [--seed <seed>] [--gpu-id <id>]
#
# Selection modes: random, grid_uniform, uniform, our_v5, deminf, subzerocore
# Results saved to: personal/work2/differentvlm/minivla/experiments/
#
# Fixed parameters:
#   - env_eval_freq: 2000
#   - save_freq: 2000
...
if [ -z "$SELECTION_MODE" ]; then
    echo "Error: --selection-mode is required"
    echo "Supported modes: random, grid_uniform, uniform, our_v5, deminf, subzerocore"
    exit 1
fi
