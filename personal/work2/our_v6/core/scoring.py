...
Action disagreement:
    ...
    - 1 sample: distance to neighbor cell representative action descriptors
...
from our_v6.core.visual_embedding import build_weighted_visual_embedding
...
def compute_action_disagreement(
    cell: AdaptiveCell,
    all_cells: Dict[str, AdaptiveCell],
    acquired_action_descriptors: Dict[int, np.ndarray],
) -> float:
    """
    Compute action disagreement for this cell using V5's action descriptors.

    Rules:
    - 0 samples: neutral default (1.0), same as visual disagreement
    - 1 sample: distance to neighboring cell representative action descriptors
    - 2+ samples: internal pairwise distance + neighbor disagreement

    All action distances are based on L2-normalized descriptors.
    ...
    cell_action_descs = []
    for ep_idx in cell.sample_episode_indices:
        if ep_idx in acquired_action_descriptors:
            cell_action_descs.append(acquired_action_descriptors[ep_idx])

    if not cell_action_descs:
        return 1.0

    if len(cell_action_descs) == 1:
        neighbors = get_neighbor_cells(cell, all_cells)
        if not neighbors:
            return 1.0
        ...
        for neighbor in neighbors:
            ...
            for ep_idx in neighbor.sample_episode_indices:
                if ep_idx in acquired_action_descriptors:
                    neighbor_descs.append(acquired_action_descriptors[ep_idx])
            if neighbor_descs:
                neighbor_rep = np.mean(neighbor_descs, axis=0)
                dist = np.linalg.norm(cell_action_descs[0] - neighbor_rep)
                neighbor_dists.append(dist)
    ...
    n = len(cell_action_descs)
    internal_dists = []
    for i in range(n):
        for j in range(i + 1, n):
            d = np.linalg.norm(cell_action_descs[i] - cell_action_descs[j])
            internal_dists.append(d)
    ...
    neighbors = get_neighbor_cells(cell, all_cells)
    neighbor_dists = []
    cell_rep = np.mean(cell_action_descs, axis=0)
...
def compute_cell_priority(
    ...
    action_disagreement = compute_action_disagreement(cell, all_cells, acquired_action_descriptors)
