#!/bin/bash
# Wait 5 hours, then check GPU memory every 30 minutes
# If GPU 1 memory < 5000MiB, launch the DemInf experiment
...
while true; do
    # Get GPU memory usage for GPU 1
    ...
    if [ "$MEMORY_USED" -lt "$MAX_MEMORY_MB" ]; then
        ...
        echo "Launching DemInf experiment..."
        echo "========================================"
        
        cd /data/zhonglinye/jun/lerobot
        conda activate lb_server
        bash personal/work2/duibi/train_and_eval_scripts/launch_deminf.sh $GPU_ID disassemble-v3_corner
        
        echo ""
        echo "DemInf experiment launched successfully!"
