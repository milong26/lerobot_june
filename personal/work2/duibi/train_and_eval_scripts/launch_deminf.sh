#!/bin/bash
# Launch DemInf experiment in tmux
# Usage: bash launch_deminf.sh <gpu_id> <dataset_name>
#   All other parameters use defaults: num_episodes=112, seed=42

set -e

GPU_ID=${1:-0}
DATASET_NAME=${2:-disassemble-v3_corner}
NUM_EPISODES=112
SEED=42

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
OUTPUT_BASE_DIR="/data/zhonglinye/jun/lerobot/personal/work2/duibi/deminf_${NUM_EPISODES}_seed${SEED}_${DATASET_NAME}"
LOG_DIR="$OUTPUT_BASE_DIR/logs"
EXP_NAME="deminf_${NUM_EPISODES}_seed${SEED}"
TMUX_SESSION="deminf_${NUM_EPISODES}_s${SEED}_${DATASET_NAME}"

# Auto-construct dataset root path from dataset name
DATASET_DIR="/data/zhonglinye/jun/lerobot/personal/work2/dataset_view/${DATASET_NAME}"

mkdir -p "$LOG_DIR"

# Kill existing session if exists
tmux kill-session -t "$TMUX_SESSION" 2>/dev/null || true

# Create runner script
RUNNER_SCRIPT="$LOG_DIR/${EXP_NAME}_runner.sh"
cat > "$RUNNER_SCRIPT" << RUNNER_EOF
#!/bin/bash

# Initialize conda
eval "\$(conda shell.bash hook 2>/dev/null)"
conda activate lb_server

# Force unbuffered Python output for real-time logging
export PYTHONUNBUFFERED=1

# Change to lerobot root directory for module imports
cd /data/zhonglinye/jun/lerobot

EXP_NAME="deminf_${NUM_EPISODES}_seed${SEED}"
GPU_ID=\$1
DATASET_NAME=\$2
OUTPUT_BASE_DIR="/data/zhonglinye/jun/lerobot/personal/work2/duibi/deminf_${NUM_EPISODES}_seed${SEED}_\${DATASET_NAME}"
LOG_DIR="\$OUTPUT_BASE_DIR/logs"
TIME_FILE="\$LOG_DIR/\$EXP_NAME.time"
PID_FILE="\$LOG_DIR/\$EXP_NAME.pid"
TRAIN_SCRIPT="/data/zhonglinye/jun/lerobot/personal/work2/duibi/train_and_eval_scripts/train_and_eval.sh"
DATASET_DIR="/data/zhonglinye/jun/lerobot/personal/work2/dataset_view/\${DATASET_NAME}"

mkdir -p "\$LOG_DIR"

# Initialize log file at the very beginning (append mode, never overwrite)
LOG_FILE="\$LOG_DIR/\$EXP_NAME.log"
touch "\$LOG_FILE"

# Redirect all stdout and stderr to log file from this point forward (append)
exec >> "\$LOG_FILE" 2>&1

DEMINF_SUBSET="/data/zhonglinye/jun/lerobot/personal/work2/deminf_results/\${DATASET_NAME}/subsets/deminf_${NUM_EPISODES}_seed${SEED}.json"

echo \$\$ > "\$PID_FILE"
echo "Start time: \$(date '+%Y-%m-%d %H:%M:%S')" >> "\$TIME_FILE"
echo "PID: \$\$" >> "\$TIME_FILE"

echo "========================================"
echo "Experiment: \$EXP_NAME"
echo "GPU: \$GPU_ID"
echo "Num Episodes: ${NUM_EPISODES}"
echo "Seed: ${SEED}"
echo "Dataset: \$DATASET_NAME"
echo "Dataset Dir: \$DATASET_DIR"
echo "PID: \$\$"
echo "Started: \$(date '+%Y-%m-%d %H:%M:%S')"
echo "========================================"
echo ""

# Check if DemInf subset already exists
if [ -f "\$DEMINF_SUBSET" ]; then
    echo "=== DemInf subset already exists, skipping selection ==="
    echo "Subset file: \$DEMINF_SUBSET"
    echo "=== Will proceed directly to fine-tune ==="
    echo ""
else
    echo "=== No pre-computed DemInf subset found, will run full pipeline ==="
    echo ""
fi

# DemInf: run full pipeline (train VAEs, compute latents, select episodes, then train)
# train_and_eval.sh will skip selection if subset already exists
bash "\$TRAIN_SCRIPT" deminf ${NUM_EPISODES} ${SEED} \$GPU_ID "" "\$DATASET_NAME" 2>&1

echo "" >> "\$TIME_FILE"
echo "End time: \$(date '+%Y-%m-%d %H:%M:%S')" >> "\$TIME_FILE"
echo "Status: completed" >> "\$TIME_FILE"

echo ""
echo "========================================"
echo "Experiment completed: \$EXP_NAME"
echo "Finished: \$(date '+%Y-%m-%d %H:%M:%S')"
echo "========================================"

exec bash
RUNNER_EOF

chmod +x "$RUNNER_SCRIPT"

# Launch in tmux
tmux new-session -d -s "$TMUX_SESSION" "bash $RUNNER_SCRIPT $GPU_ID $DATASET_NAME"

echo "Launched experiment: $EXP_NAME"
echo "tmux session: $TMUX_SESSION"
echo "Output dir: $OUTPUT_BASE_DIR"
echo "Dataset: $DATASET_NAME"
echo "Dataset dir: $DATASET_DIR"
echo ""
echo "Monitor with: tmux attach -t $TMUX_SESSION"
echo "Check logs: tail -f $LOG_DIR/$EXP_NAME.log"