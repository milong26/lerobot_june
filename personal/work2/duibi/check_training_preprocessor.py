#!/usr/bin/env python3
"""
模拟训练脚本的行为，检查 preprocessor 是否正确归一化了 action。
"""
...
print("=" * 60)
print("模拟训练脚本的行为，检查 preprocessor 的 action 归一化")
print("=" * 60)
...
# 模拟训练脚本的 preprocessor_overrides
...
print(f"\n正在使用 make_pre_post_processors 创建 preprocessor...")
print(f"  pretrained_path: None（模拟从头创建）")
print(f"  preprocessor_overrides: {list(preprocessor_overrides.keys())}")

# 使用 make_pre_post_processors 创建 preprocessor（模拟训练脚本的行为）
preprocessor, postprocessor = make_pre_post_processors(
    policy_cfg=config,
    pretrained_path=None,  # 模拟从头创建
