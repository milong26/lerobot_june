...
│   ├── action_embedding.py # Action trajectory embedding 提取
...
1. **Embedding 空间扩展**: v1 仅使用 global+wrist 视觉 embedding；v2 可选加入 action trajectory embedding (x_t, v_t, hidden representation)，形成 state-action 联合空间。
