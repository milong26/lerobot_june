#!/bin/bash
# GPU监控脚本：6小时后开始检查GPU 1显存，满足条件后自动启动TinyVLA-B训练任务
# 使用方法: bash monitor_gpu_and_launch_tinyvla_b.sh

set -e

# 配置参数
INITIAL_WAIT_HOURS=6          # 初始等待时间（小时）
CHECK_INTERVAL_MINUTES=20     # 检查间隔（分钟）
GPU_TARGET=1                  # 目标GPU ID
MEMORY_THRESHOLD=5000         # 显存阈值（MiB）
LAUNCH_SCRIPT="personal/work2/differentvlm/scripts/run_tinyvla.sh"
LAUNCH_ARGS="tinyvla-b --dataset disassemble-v3_corner --gpu 1 --num_episodes 112"
...
LOG_FILE="$LOG_DIR/monitor_tinyvla_b_${TIMESTAMP}.log"
...
main() {
    ...
    log "  - 启动脚本: ${LAUNCH_SCRIPT}"
    log "  - 启动参数: ${LAUNCH_ARGS}"
    log "  - 日志文件: ${LOG_FILE}"
    log "========================================="
