    ...
    with torch.no_grad():
        ...
        for batch_idx in range(n_batches):
            ...
            for frame in batch_frames:
                ...
                inputs = processor(
                    text="<image> Pick and place a puck to a goal",
                    images=frame,
                    return_tensors="pt"
                ).to(device)
...
def process_dataset(
    ...
    for ep_idx in range(dataset.num_episodes):
        ...
        # 检查是否已存在缓存
        coord_key = f"({ep_idx})"
        if coord_key in existing_files:
