...
## Fine-tuning SmolVLA with periodic multi-task evaluation

The repository's RoboMME training wrapper fine-tunes `lerobot/smolvla_base` on a merged
RoboMME dataset and can evaluate several RoboMME tasks during training. By default it
evaluates `MoveCube,PatternLock,RouteStick` every 2,000 updates on five fixed test
episodes per task.

```bash
bash personal/work2/duibi_robomme/train_and_eval_scripts/train_smolvla_robomme.sh \
  --merged-dataset-dir <MERGED_ROBOMME_DATASET> \
...
The training dataset cameras are renamed from
`observation.images.image/wrist_image` to the SmolVLA checkpoint's
`camera1/camera2` feature names. The same configured rename map is reused by
training-time environment evaluation.
