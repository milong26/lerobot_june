...
def convert_data(
    ...
    features_dict = hf_features.to_dict()
    metadata_json = json.dumps({"features": features_dict})
    schema_metadata = {b"info": metadata_json.encode("utf-8")}
    ...
    for (chunk_idx, file_idx), records in tqdm.tqdm(
        ...
        for record in records:
            ...
            new_table = episode_table.replace_schema_metadata(schema_metadata)
            pq.write_table(new_table, dest_path)
...
def _discover_video_keys_from_records(
    episode_records: list[dict[str, Any]],
) -> list[str]:
...
def convert_videos(
    ...
    for video_key in video_keys:
        ...
        if len(grouped) == 0:
            raise KeyError("No video metadata found for key '%s'; skipping", video_key)
            continue
...
def convert_dataset(
    ...
    video_keys = _discover_video_keys_from_records(episode_records)
