...
def tokenizer_image_token(prompt, tokenizer, image_token_index=IMAGE_TOKEN_INDEX, return_tensors=None):
    """Tokenize a prompt that may contain <image> placeholder(s).

    This function splits the prompt by '<image>', tokenizes each chunk separately,
    and inserts the IMAGE_TOKEN_INDEX (-200) between chunks. This ensures the
    LLaVA architecture can properly replace image tokens with visual features.

    Args:
        prompt: The text prompt, potentially containing '<image>' placeholder(s).
        tokenizer: The tokenizer instance.
        image_token_index: The token index to use for image placeholders (default: -200).
        return_tensors: If 'pt', return a PyTorch tensor.

    Returns:
        List of token IDs, or torch.Tensor if return_tensors='pt'.
    """
